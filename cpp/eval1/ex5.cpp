...
bool cherche_mot_dans_mot(const char mot_dans[], const char mot_quoi[],
                          const size_t taille_mot_dans,
                          const size_t taille_mot_quoi) {
...
bool cherche_mot_dans_grille(const char grille[][C], const char mot[]) {
  ...
  for (size_t ligne = 0; ligne < L and taille_mot <= L; ++ligne) {
    ...
    for (size_t i = 0; i < C; ++i)
      mot_ligne[i] = grille[ligne][i];
    if (cherche_mot_dans_mot(mot_ligne, mot, C, taille_mot))
      return true;
  }
  ...
  for (size_t colonne = 0; colonne < C and taille_mot <= C; ++colonne) {
    ...
    for (size_t i = 0; i < L; ++i)
      mot_colonne[i] = grille[i][colonne];
    if (cherche_mot_dans_mot(mot_colonne, mot, L, taille_mot))
      return true;
  }
  return false;
}
