...
void affecterChaine(Chaine &chaine, const char *str) {
  size_t taille_str = 0;
  for(const char *ptr = str; *ptr != '\0'; ++ptr) ++taille_str;
...
void concatenationChaines(Chaine &chaine_1, const Chaine &chaine_2) {
  ...
  while (*i_chaine_2 != '\0') *i_chaine_1++ = *i_chaine_2++;
  *i_chaine_1 = '\0';
}

char enMajuscule(const char c) {
  if('a' <= c && c <= 'z') return c-32; // 'a'-'A'=32
  else return c;
}

void appliqueAUneChaine(Chaine &chaine, char(*func)(char)) {
  for(char *ptr = chaine.tampon; *ptr != '\0'; ++ptr) *ptr = func(*ptr);
}
